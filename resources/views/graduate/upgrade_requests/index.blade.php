                    ...
                    <tbody>
                        @forelse ($upgradeRequests as $request)
                            <tr>
                                ...
                                <td>{{ Str::limit($request->reason, 50) ?? 'N/A' }}</td>
                                <td>
                                    <span class="badge
                                ...
                                <td class="text-end pe-3">
                                    <div class="btn-group btn-group-sm">
                                        ...
                                        @if($request->status === 'pending')
                                            {{-- يمكن إضافة أزرار موافقة/رفض سريعة هنا إذا أردت --}}
                                        @endif
                                         {{-- زر حذف الطلب (اختياري) --}}
                                        <form action="{{ route('admin.upgrade-requests.destroy', $request) }}" method="POST" class="d-inline" onsubmit="return confirm('Delete this request record? This will not change user role if already approved/rejected.');">
                                            @csrf
                                            @method('DELETE')
